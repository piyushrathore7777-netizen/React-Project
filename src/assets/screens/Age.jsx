import React from "react";
import image5 from "../Age3image.png";
import { Link } from "react-router";





export default (CardA) => {
  return (

    <body className='bg-black from-black via-gray-900 to-black text-white  flex-col 
  items-center justify-center '>
        <nav className="flex items-center justify-between p-6">
            <div className="flex justify-around items-center gap-3 px-7">
               <div className='border border-white hover:border-hidden cursor-pointer rounded-full '>
          <button className='py-2 cursor-pointer '>
            <img className="h-8  px-2" src="data:image/png;base64," alt="" srcset="" />
          </button>
        </div>
        <div className="px-2">
            <p className="text-3xl font-semibold cursor-pointer">BetterMe</p>
            </div>
            </div>
             <div className='border border-white hover:border-hidden cursor-pointer rounded-full '>
          <button className='py-2 cursor-pointer '>
            <img className="h-8  px-2" src="data:image/png;base64," alt="" srcset="" />
          </button>
        </div>


        </nav>
        <hr></hr>

        <section className="flex items-center justify-evenly mt-15 px-20">
          <div className="flex justify-between  mt-4 ">
          <div className="py-22 px-15">
            <p className="text-5xl font-semibold">Over 430,000 men</p>
            <p className="text-3xl ">in their 20s have already tried our <br /> Calisthenics Workout Plan</p>
          </div>

          <div className="justify-around items-center h-20 ">
        
              <img src={image5} className="h-80 w-130 px-15" alt="" srcset="" />
          </div>
          </div>
        </section>
        
        
        
        <section className="justify-around items-center py-22">
          <div>
          <hr></hr>
          </div>
          <div className="px-140 py-6 ">
         <Link to="/continue">
          <button className="bg-amber-50 w-90 rounded-full h-12 text-black font-semibold cursor-pointer">CONTINUE</button>
         </Link>
          </div>
        </section>


    </body>
  )
}

