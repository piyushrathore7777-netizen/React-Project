import React from 'react'
import image from '../image1.webp';
import Arrow from '../Arrow(1).svg';
import image2 from '../image2.webp';
import image3 from'../image3.webp';
import image4 from '../image4.webp';


export default function GetStarted() {
   return (
  <body className='bg-gradient-to-br from-black via-gray-900 to-black text-white  flex-col 
  items-center justify-center '>
    <section className='flex py-2 items-center justify-center gap-285'>
    <div className='top-6 left-8 text-2xl font-semibold cursor-pointer'>
      BetterMe
    </div>
    <div className='border border-white hover:border-hidden cursor-pointer rounded-full '>
      <button className='py-2 cursor-pointer '>
          <img className="h-5  px-2" src="data:image/png;base64," alt="" srcset="" /> 
              </button>
    </div>
    </section>
    <section className='py-10'>
      <center>
    <h1 className='text-4xl md:text-5xl font-bold tracking-wide'>
      CALISTHENICS WORKOUT PLAN
    </h1>
    <p className='mt-3 text-gray-400 text-lg'>
      BASED ON YOUR AGE
    </p>
    </center>
    </section>



     <section className='flex justify-around items-center px-48 py-10'>

      {/* caard */}

      <div className='bg-gray-500 rounded-2xl w-70 h-72 '>
        <div className='justify-center items-center py-2 relative overflow-hidden shadow-lg hover:scale-105 transition duration-300'>
        <img src={image} className='h-58 w-70 overflow-hidden cursor-pointer' alt="" srcset="" />
        </div>
        <div className='px-2 absolute overflow-hidden self-center items-center justify-evenly '>
        <button className='bg-green-50 rounded-2xl w-65 h-15 space-x-25 overflow-hidden cursor-pointer'>
          <span className='font-medium text-black text-xl'>Age:18-29</span>
          <button><img src={Arrow} width={20} alt="" srcset="" />  </button>
        </button>
        </div>

      </div>

      <div className='bg-gray-500 rounded-2xl w-70 h-72'>
        <div className='justify-center items-center py-2 relative overflow-hidden shadow-lg hover:scale-105 transition duration-300'>
        <img src={image2} className='h-58 w-70 overflow-hidden cursor-pointer' alt="" srcset="" />
        </div>
        <div className='px-2 absolute overflow-hidden self-center items-center justify-evenly '>
         
        <button className='bg-green-50 rounded-2xl w-65 h-15 space-x-25 overflow-hidden cursor-pointer'>
          <span className='font-medium text-black text-xl'>Age:30-39</span>
          <button ><img src={Arrow} width={20} alt="" srcset="" />  </button>
        </button>
        </div>
        </div>


    <div className='bg-gray-500 rounded-2xl w-70 h-72'>
        <div className='justify-center items-center py-2 relative overflow-hidden shadow-lg hover:scale-105 transition duration-300'>
        <img src={image3} className='h-58 w-70 overflow-hidden cursor-pointer' alt="" srcset="" />
        </div>
        <div className='px-2 absolute overflow-hidden self-center items-center justify-evenly '>
         
        <button className='bg-green-50 rounded-2xl w-65 h-15 space-x-25 overflow-hidden cursor-pointer'>
          <span className='font-medium text-black text-xl'>Age:40-49</span>
          <button><img src={Arrow} width={20} alt="" srcset="" />  </button>
        </button>
        </div>
        </div>


     <div className='bg-gray-500 rounded-2xl w-70 h-72'>
        <div className='justify-center items-center py-2 relative overflow-hidden shadow-lg hover:scale-105 transition duration-300'>
        <img src={image4} className='h-58 w-70 overflow-hidden cursor-pointer' alt="" srcset="" />
        </div>
        <div className='px-2 absolute overflow-hidden self-center items-center justify-evenly '>
         
        <button className='bg-green-50 rounded-2xl w-65 h-15 space-x-25 overflow-hidden cursor-pointer'>
          <span className='font-medium text-black text-xl'>Age:50</span>
          <button><img src={Arrow} width={20} alt="" srcset="" />  </button>
        </button>
        </div>
        </div>



     </section>

     <section className='flex justify-center items-center'>
      <p className='font-semibold'>By choosing your age and continuing you agree to our <span className='text-blue-500 underline-offset-1'>Terms of Service</span>  | <span className='text-blue-500 underline-offset-1'>Privacy Policy</span> <br />
     <center> Please review before continuing </center></p>
     </section>

    
     
  </body>
  )
}
